...
- [2024.11]🎯[新功能]: 推出 LightRAG WebUI —— 一个允许您通过直观的 Web 界面插入、查询和可视化 LightRAG 知识的仪表板。（注：本分支中打包的 WebUI 仅用于查询和可视化，文档插入与图谱编辑相关的 UI 已被有意移除。）
...
LightRAG 服务器提供了一个供体验LightRAG功能的Web UI，以及一个按蓝绿知识图谱生成分区（plane）划分的只读 `REST API`，仅限查询与图谱浏览；文档写入通过外部调用方使用类型化 SDK 合约在服务器之外完成，并不经过该服务器。有关LightRAG服务器的更多信息，请参阅[LightRAG服务器](./docs/LightRAG-API-Server.md)。
...
LightRAG 的默认查询模式为 mix。使用 mix 模式通常可以获得最为理想的查询结果。mix 模式比 naive 耗时略长；其他查询模式在耗时上基本相当。**注意**：本分支按知识图谱生成分区（plane）划分的 REST API（`/planes/{plane}/query` 等接口）仅接受 `local`、`global`、`hybrid` 和 `mix`；`naive`（以及内部的 `bypass` 模式）仅可通过 SDK 使用。
...
LightRAG 的默认存储全部都是基于文件进行持久化的内存数据库。默认存储仅用于开发调试，不适合用于生产环境部署。生产环境如果希望使用同一个后台数据解决 4 种类型的后台存储，可以选择 PostgreSQL、MongoDB 或 OpenSearch。也可以单独为向量存储或图存储选择专业化的数据库，例如使用 Milvus 或 Qdrant 作为向量存储，使用 Neo4j 或 Memgraph 作为图存储。**注意**：以上是通用 SDK/核心层面的说明。本分支的 `lightrag-server` 始终通过 `create_postgres_generation_runtime()` 构建生成运行时，要求全部 4 种存储角色均使用 PostgreSQL；MongoDB 和 OpenSearch 仅在直接使用 SDK 时可用，无法用于运行 `lightrag-server`。
