...
def mock_pg_db():
    ...
    db.hnsw_m = 16
    db.hnsw_ef = 64
    db.ivfflat_lists = 100
    db.vchordrq_build_options = ""
    connection = AsyncMock()
    connection.fetchval = AsyncMock(return_value=None)

    async def run_with_retry(operation, **kwargs):
        return await operation(connection)

    db._run_with_retry = AsyncMock(side_effect=run_with_retry)
    db._test_connection = connection
...
async def test_postgres_halfvec_table_creation(
    ...
    create_table_calls = [
        call
        for call in mock_pg_db._test_connection.execute.call_args_list
